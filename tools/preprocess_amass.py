...
motion_data = []
with open(all_motions, "rb") as pkl_file:
    while True:
        try:
            motion_data.append(pickle.load(pkl_file))
        except EOFError:
            break
...
N = 0
max_len = 2916
with open('data/AMASS/amass_joints_h36m_60.pkl','wb') as pkl_file:
    with open('data/AMASS/clip_list.csv', 'w') as f:
        ...
        for i, bdata in enumerate(motion_data):
            ...
            for sid in range(num_slice+1):
                ...
                pickle.dump(kpts, pkl_file)
                print(N, ',', fname_list[i], ',', end-start, file=f)
                N += 1
